...
**Solution:** Seperate the `loading` states into `employeesLoading` and `paginatedTransactionLoading`
...
**Solution:** Conditional rendering for `View More` Button. Only when `paginatedTransactions` is not `null` is this button displayed.
...
**Solution:** Take advantage of the cache. Use cache in the TransactionPane instead of fetching the data from the server every time.
